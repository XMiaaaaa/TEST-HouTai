...
const ExamDetailModal: React.FC<ExamDetailModalProps> = ({ isOpen, onClose, examId }) => {
  // 模拟考试记录数据
  const [exam, setExam] = useState<ExamRecord>({
    id: 1, 
    examDate: '2026-04-15', 
    location: '科目一考场', 
    coaches: ['张教练'], 
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="考试详情">
      {/* 考试基本信息 */}
                ...
                {examStudents.map((student, index) => (
                  <tr key={student.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`status-badge ${student.status === '通过' ? 'status-通过' : 'status-挂科'}`}>
                        {student.status}
                      </span>
                    </td>
